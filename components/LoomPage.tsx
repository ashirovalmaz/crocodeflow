...
import { CheckCircle2, ArrowRight, PlayCircle, Video, ListChecks, Lock } from 'lucide-react';
...
export const LoomPage: React.FC<LoomPageProps> = ({ previewData, themeMode }) => {
  ...
  useEffect(() => {
    ...
    style.innerHTML = `
      ...
      ${scope}.text-brand-700 { color: ${darker} !important; } 
      ...
      ${scope}.bg-brand-400 { background-color: ${lighter} !important; }
      ${scope}.bg-brand-500\\/5 { background-color: ${color}0d !important; }
      ...
      ${scope}.border-brand-200 { border-color: ${lighter}40 !important; }
      ...
      ${scope}.from-brand-600 { --tw-gradient-from: ${darker} !important; var(--tw-gradient-stops, var(--tw-gradient-from), var(--tw-gradient-to)) !important; }
      ...
      ${scope}.to-brand-200 { --tw-gradient-to: ${lighter}80 !important; var(--tw-gradient-stops, var(--tw-gradient-from), var(--tw-gradient-to)) !important; }
      
      /* Shadows */
      ${scope}.shadow-brand-500\\/25 { --tw-shadow-color: ${color}40 !important; }
      ${scope}.shadow-brand-500\\/30 { --tw-shadow-color: ${color}4d !important; }
      ...
      ${scope}.dark .dark\\:bg-brand-500\\/5 { background-color: ${color}0d !important; }
      ${scope}.dark .dark\\:bg-brand-500\\/10 { background-color: ${color}1a !important; }
      ...
      ${scope}.dark .dark\\:from-brand-400 { --tw-gradient-from: ${lighter} !important; var(--tw-gradient-stops, var(--tw-gradient-from), var(--tw-gradient-to)) !important; }
      ${scope}.dark .dark\\:to-brand-200 { --tw-gradient-to: ${lighter}40 !important; var(--tw-gradient-stops, var(--tw-gradient-from), var(--tw-gradient-to)) !important; }
      
      /* Other */
      ${scope}.fill-brand-500\\/20 { fill: ${color}33 !important; }
      ${scope}.hover\\:bg-brand-500:hover { background-color: ${color} !important; }
      ${scope}.hover\\:border-brand-400\\/50:hover { border-color: ${lighter}80 !important; }
    `;
  ...
  return (
    <div className={`custom-theme-scope ${themeMode || ''} w-full isolate`}>
        <div className={`
            bg-gray-50 dark:bg-dark-950 text-gray-900 dark:text-white transition-colors duration-300 flex flex-col relative
        ...
        <main className={`flex-grow px-4 md:px-6 relative z-10 flex flex-col items-center ${previewData ? 'pt-24 pb-12' : 'pt-32 pb-24'}`}>
            ...
            <div className="mb-12 text-center animate-fade-in">
                <div className="inline-flex items-center gap-2 px-4 py-1.5 mb-8 rounded-full bg-brand-50 dark:bg-brand-500/5 border border-brand-200 dark:border-brand-500/20 text-brand-700 dark:text-brand-400 text-xs font-bold uppercase tracking-widest backdrop-blur-md shadow-sm transform hover:scale-105 transition-transform duration-300 cursor-default">
                ...
                <h1 className="text-4xl md:text-5xl lg:text-7xl font-display font-bold text-gray-900 dark:text-white mb-6 tracking-tight leading-[1.1] drop-shadow-sm">
            ...
            <div className="flex flex-col gap-10">
                ...
                <div className="w-full max-w-5xl mx-auto group relative rounded-2xl overflow-hidden border border-gray-200 dark:border-dark-700 shadow-2xl bg-black animate-slide-up">
                    {/* Glow Effect */}
                    <div className="absolute -inset-1 bg-gradient-to-r from-brand-500 to-brand-300 rounded-2xl blur opacity-20 group-hover:opacity-40 transition duration-1000 group-hover:duration-200"></div>
                    
                    {/* 16:9 Aspect Ratio Container */}
                    <div className="relative bg-black rounded-xl overflow-hidden shadow-inner" style={{ position: 'relative', paddingBottom: '56.25%', height: 0 }}>
                        {pageData.videoId ? (
                            <iframe 
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-slide-up w-full max-w-5xl mx-auto" style={{ animationDelay: '0.2s' }}>
                    ...
                    {/* Highlights Card */}
                    <div className="lg:col-span-7 p-8 bg-white dark:bg-dark-900/40 backdrop-blur-md rounded-2xl border border-gray-200 dark:border-dark-700 shadow-sm flex flex-col justify-center hover:border-brand-500/30 transition-colors">
                        ...
                        <ul className="space-y-4">
                            {pageData.text?.highlights?.map((item, i) => (
                                <li key={i} className="flex items-start gap-3 group">
                                    <div className="mt-0.5 rounded-full bg-brand-50 dark:bg-brand-500/10 p-1 group-hover:bg-brand-100 dark:group-hover:bg-brand-500/20 transition-colors">
                    ...
                    {/* CTA Card */}
                    <div className="lg:col-span-5 p-8 bg-gray-900 dark:bg-gradient-to-br dark:from-dark-800 dark:to-dark-900 rounded-2xl border border-gray-800 dark:border-dark-700 shadow-xl flex flex-col justify-center items-start relative overflow-hidden group">
                        {/* Ambient Glow */}
                        ...
                        {/* Lock Icon bg */}
                        <Lock className="absolute bottom-4 right-4 w-24 h-24 text-white/5 rotate-[-15deg] pointer-events-none" />

                        <h3 className="font-display font-bold text-2xl mb-3 text-white relative z-10">
                            {pageData.text?.ctaTitle}
                        </h3>
                        <p className="text-gray-400 text-sm mb-8 relative z-10 leading-relaxed max-w-sm">
